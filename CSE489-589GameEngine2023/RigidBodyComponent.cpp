...
RigidBodyComponent::~RigidBodyComponent()
{
	...
	if (owningGameObject->gameObjectName == "alien") {
	...
	if (collisionData->bulletRigidBody != this->ghostRigidBody &&
		...
		for (auto& bud : buddys) {
			...
			if (bud != shared_from_this()) {
				...
				if (bud->owningGameObject->gameObjectName == "projectile") {
					int randomval = std::rand();
					if (randomval % 2 == 0) {
						std::shared_ptr<SoundSourceComponent> sfx =
							std::make_shared<SoundSourceComponent>("Sounds/Hit1.wav", 10.0f, .10f, 100.0f, 200);
						sfx->setLooping(false);
						sfx->play();
						sfx->setGain(1);
						owningGameObject->addComponent(sfx);
					}
					else {
						std::shared_ptr<SoundSourceComponent> sfx =
							std::make_shared<SoundSourceComponent>("Sounds/Hit2.wav", 10.0f, .10f, 100.0f, 200);
						sfx->setLooping(false);
						sfx->play();
						sfx->setGain(1);
						owningGameObject->addComponent(sfx);
				...
				if (bud->owningGameObject->gameObjectName == "alien") {
					
				}
				owningGameObject->addComponent(std::make_shared<RemoveComponent>(2.0));
			}
		}
	}
