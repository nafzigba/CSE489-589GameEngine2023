...
class Scene2 : public Game
{
	...
	void loadScene() override
	{
		...
		//std::shared_ptr<class GameObject> sphereGameObject3 = std::make_shared<GameObject>();
		//this->addChildGameObject(sphereGameObject3);
		////sphereGameObject3->setPosition(vec3(70.7f,0.0f, 70.7f), WORLD);
		////sphereGameObject3->setPosition((cylinderGameObject->getPosition(WORLD) + boxGameObject->getPosition(WORLD))/2.0f, WORLD);

		//Material sphereMat3;
		//sphereMat3.setDiffuseMat(vec4(0.0f, 1.0f, 0.0f, 1.0f));
		//std::shared_ptr<SphereMeshComponent> sphereMesh3 = std::make_shared<SphereMeshComponent>(shaderProgram, sphereMat3, 5.0f, 24, 32);
		//sphereGameObject3->addComponent(sphereMesh3);

		//std::shared_ptr<RigidBodyComponent> sgorg3 = std::make_shared<RigidBodyComponent>(sphereMesh3, STATIONARY, 1000.0f);
		//sgorg3->setGravityOn(false);
		//sphereGameObject3->addComponent(sgorg3);

		//sphereGameObject3->gameObjectName = "green sphere - STATIONARY";
		//sphereGameObject3->addComponent(std::make_shared<CollisionComponent>());
